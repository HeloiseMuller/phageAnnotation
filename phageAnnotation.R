...
option_list = list(
  ...
  make_option(c("-l", "--len"), type="numeric", default=5000, 
              help="filter out phage sequences shorter than this threashold [default = 5000]", metavar="numeric"),
  make_option(c("-f", "--filtering"), type="numeric", default=0, 
              help="whether should apply tuning removal filters [default = 0 to not apply; 1 applies them]", metavar="numeric"),
  make_option(c("-o", "--out"), type="character", default=NULL, 
              help="path where to save the outputs. If none is providing, ouputs will be save at the same location as checkV file", metavar="character")
); 
...
quality_phage_f <- function(dt, both="no"){ #both ask whether info about both 1st and 2nd run of VS or just 2nd
  ...
  if(both=="no"){
    ...
    dt[, keep_Sullivan := ifelse(keep_tmp!="others", keep_tmp,
                                 ifelse(keep_tmp=="others" & viral_genes_checkV==0 & host_genes_checkV==1 & length.y>=10000, "manual", "discard"))]
  }
  dt <- dt[, -"keep_tmp"]
  ...
  if(both=="no"){
    dt[, Hegarty_addition_1 := ifelse(percUnknown>=75 & length<50000, 0.5, 0)] #LENGTH contig or virus? 
  } else {
    dt[, Hegarty_addition_1 := ifelse(percUnknown>=75 & length.y<50000, 0.5, 0)] 
  }
  ...
  if(both=="no"){
    ...
    dt[, Hegarty_removal_4 := ifelse(length.y>50000 & hallmark<=1, -3, 0)]
  }
...
fwrite(phages_filtered, paste0(outF, "phages_filtered.tsv"), sep = "\t")

######################
#Save a list of seqname to keep so easy to remove them from fasta
fwrite(phages_filtered[, "seqname"], paste0(outF, "phages_filtered.lst"), sep = "\t", col.names = F)
